...
function move() {
    ...
    function frame() {
        ...
        if (width >= 100) {
            var welcome = new Audio('os_sounds/welcome.wav'); //Preload audio
			welcome.play();
			$( "#logo" ).fadeOut( 2000 );
...
function breakFrameCode_result(){
	...
	var console = '<p id="code">Welcome to Nulwo 2.0...<br>Type `help` for a list of commands</p><div class="command" id="command" style="visibility:visible;"><form id="norman" action="javascript:blank(null);">/<input id="input" autocomplete="off" type="text" class="blink_me" placeholder="█" ></form></div>';
...
var error = new Audio('os_sounds/error.wav'); //Preload audio
function errorwd(){
	$("#errorwd").show();
	error.play();
}
      ...
      $("body").css("color", colour);
       $("#input").css("color", colour);
			...
			$('#input').bind('keydown.return', function(){
				...
				space();
				space();
				lines("7▓89611.╩t///HEY!! YOU!! TRY OPENING THAT FILE!");
				lines("USE '/open key.jpg'///?▒s4?▒s4");
				stop(1);
				scrollbottom();
				}, 3000);
				...
				stop(0); 
				space();
				lines("Opening 'G.bio' with 'Nul-World.exe'");
				lines("");
				scrollbottom();
				delay = setTimeout(function(){ lines("ERROR: G.bio is running already in another location...")}, 3000);
				delay = setTimeout(function(){ lines("Transmitted running iteration succesfully! G.bio can be displayed using nulWorld.exe...");
				lines("Please type in '/nulWorld.exe G.bio' to display your 'G.bio'");
				stop(1);
				scrollbottom();}, 6000);
				...
				if (x=="rnulWorld.exe G.bio" || x=="nulworld.exe G.bio" || x=="nulworld.exe g.bio"){
				stop(0);  
				space();
				lines("Running...");
				...
				if (x=="exit"){
				window.location = 'https://blank-null.tumblr.com';
				return false;
				} else if(x!="" || x!=" "){
				stop(0);
				lines("Error 001: Command was wrongly inputed or doesn't exist, try again.");
				stop(1);
				scrollbottom();
...
function protocol001(){
	...
	$("body").html('<div class="ALERT"><div id="ALERT">ALERT<br>UNKNOWN SCRIPT EXECUTING FROM THIRD SOURCE<br>REDIRECTING TO 10.5.95.1</div></div>');
	...
	setTimeout(						
	function(){	$("body").html('');
				window.location = "10.5.95.1.html";
				},
				17000);
